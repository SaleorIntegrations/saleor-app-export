...
import { OrderFieldEnum, ExportObjectTypesEnum } from '../api/export/types'
...
export const useExportOrder = () => {
  ...
  if (!('orderFields' in state.columns)) {
    throw new Error('Export Context is type of Product')
  }

  const setOrderFields = (orderFields: OrderFieldEnum[]) =>
    dispatch({ action: 'SET_COLUMNS', columns: { orderFields: orderFields } })
