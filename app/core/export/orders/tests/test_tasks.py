...
async def test_init_export_for_report(m_headers, m_continue, db_session, orders_report):
    ...
    # when
    await init_export_for_report(db_session, orders_report.id, OrderExportMethods)
    ...
    m_continue.assert_awaited_once_with(mock.ANY, export.id, OrderExportMethods)
...
async def test_continue_export_with_empty_cursor(
    ...
    # when
    await continue_export(db_session, orders_export.id, OrderExportMethods)
